...
const profiles  = ref([
{
  clinic_id: "歯科医療機関ID",
  division: "区分***",
  clinics_number: "歯科医療機関会員番号",
  clinic_name: "****歯科クリニック",
  clinic_phonetic: "よみがな",
  clinic_phonetic_spelling: "よみがな英字",
  email: "メール***",
  password: "パスワード***",
...
<template>
  <v-card elevation="2" v-for="item in profiles" :key="item.clinic_id">
    <v-card-text class="pa-0">
      <div class="bg-secondary pa-4">
        <h2 class="">歯科医療機関 情報</h2>
        <h4 class="font-weight-light">Check clinic's Prolile !</h4>
      ...
      <v-alert
        ...
        <v-row class="ma-4">
          <v-col cols="12" xl="6" lg="6" md="6" class="ml-lg-6 d-flex justify-start">
            <h3>医療機関名 : {{ item.clinic_name }}</h3>
          </v-col>
          <v-col cols="12" xl="6" lg="6" md="6" class=" d-flex justify-start">
              <div class="ml-lg-10 pl-lg-8">英字: {{ item.clinic_phonetic_spelling }} </div>
          </v-col>
        ...
        <div v-if="!(users.usertype === 'patient')">
          ...
          <v-col cols="12" class="ml-lg-6 d-flex justify-start">
            <h3>専門分野 : </h3>
            <div class="ml-4">{{ item.speciality_clinic }} </div>
          </v-col>
            <v-divider></v-divider>
          ...
          <v-col cols="12" class="ml-lg-6 mb-4">
            <v-card class="d-flex mx-4">
              ...
              <v-card class="d-flex justify-end pr-8">
                <v-btn
                  v-if="users.usertype === 'clinic'"
                  to="/mypages/artisan/setting4"
                  size="small"
                  color="error"
                  variant="flat"
                  elevation="4">
